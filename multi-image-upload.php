...
class Multi_Image_Uploader
{
    ...
    {
        $this->post_types = array('post', 'page', 'lf_event', 'lf_programme');     //limit meta box to certain post types
    ...
    public function save($post_id)
    {
        ...
        $posted_titles = $_POST['miu_images_title'];
        
        $miu_images = $miu_titles = array();
        if (!empty($posted_images))
        {
            foreach ($posted_images as $index => $image_url)
            {
                if (!empty($image_url)){
                  $miu_images[] = esc_url_raw($image_url);
                  $miu_titles[] = $posted_titles[$index];
                }
        ...
        update_post_meta($post_id, 'miu_titles', serialize($miu_titles));
    }
    ...
    public function render_meta_box_content($post)
    {
        ...
        $titles = get_post_meta($post->ID, 'miu_titles', true);

        //print_r($titles);

        $metabox_content = '<div id="miu_images"></div><input type="button" value="Add Image" class="button multi_btn" />';
        ...
        if (!empty($images))
        {
            ...
            {
                $script.="addRow('".$image."','".$titles[$index]."');";
            }
        }
        $script .="</script>";
        echo $script;
    ...
    function enqueue_scripts($hook)
    {
        ...
        wp_enqueue_style( 'multiupload-style', plugin_dir_url(__FILE__) . 'upload.css' );
    }
